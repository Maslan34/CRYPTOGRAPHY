...
FILE *readingPtr,*writingPtr;
	...
	//sorted by possibility of existing a character 
	...
	for(i=0;i<26;i++)
		...
		for(i= 0; i< 26; i++) 
		{
		    printf("%.2f  ", Sorted_Occurrencies_Letters[i]); to see it
...
*/
	...
	//DECRYPTION
	if( ((writingPtr=fopen("Predefined-Rates.txt","w"))==NULL) || ((readingPtr=fopen("CipherText.txt","r"))==NULL))
		printf("OPENING FILE ERROR!");
	else
	{
		while(!feof(readingPtr))
